...
- [x] Security: widget origin allowlist (PR #3, merged + deployed to production). Previously any third-party site could embed any org's widget just by knowing its (public, non-secret) slug — all 7 widget-facing routes hardcoded `Access-Control-Allow-Origin: "*"` with zero validation of the calling page. Added `Organization.allowedOrigins String[] @default([])` — empty means unrestricted (matches prior behavior exactly, zero-risk default). New shared `apps/web/lib/cors.ts` (`corsHeaders()`, `checkOriginAllowed()`) replaces the 7 duplicated inline `corsHeaders` consts across `posts/route.ts`, `posts/[id]/vote/route.ts`, `posts/[id]/comments/route.ts`, `changelog/route.ts`, `roadmap/route.ts`, `api/widget/[org]/config/route.ts`, `api/widget/[org]/identify/route.ts`. `checkOriginAllowed()` rejects (403, no CORS headers on the rejection so the browser blocks the attacker page from reading it) only when an `Origin` header IS present AND it doesn't match the app's own origin AND the org's allowlist is non-empty AND nothing matches. Requests with no `Origin` header (server-to-server calls) and orgs that haven't opted in (empty allowlist) are always allowed. **Post-review fix (357cb60):** the app's own origin (`NEXT_PUBLIC_APP_URL`) is now *always* allowed regardless of the allowlist — browsers send `Origin` on same-origin POST/DELETE fetches too, so without this exemption an org's own public feedback/vote/comment calls started 403ing the moment it added any third-party embed origin. Caught in PR review before merge, fixed + regression-tested (24/24 tests passing) before shipping. Settings → Allowed Origins UI (`AllowedOriginsSection.tsx`) added, bundled into the existing Organization save flow (`useOrgSettings.ts`) — copy explicitly notes the org's own pages are never affected. Demo org (`freebase`, used for the live widget on both `localhost:3000` and `https://freebase.vercel.app`) seeded via a new **non-destructive** `packages/db/prisma/set-allowed-origins.ts` (`pnpm --filter db set-origins`) — deliberately kept separate from the existing `seed.ts`, which wipes and reseeds all feedback/changelog/roadmap data and must never be run against production for this purpose. Note: `explain-like-i-am-5.vercel.app` and any other org living outside this monorepo/Vercel project will NOT get this fix from merging here — it needs its own independent deploy if it runs a separate codebase.
